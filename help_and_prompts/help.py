...
f = open(os.path.join(os.path.dirname(__file__), 'help_topics.json'), encoding='utf-8')
data = json.load(f)
data = data['intents']
...
def get_help(user_name):
    
    abriella_speak(f"ok {user_name}, what do you need help with?")
    ...
    while True:
        ...
        if question == "exit":
                abriella_speak(f"""Ok {user_name}, I've exited the help section, if you need to continue, just say get help again,
